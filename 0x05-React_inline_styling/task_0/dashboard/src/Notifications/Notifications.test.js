...
describe('<Notifications />', () => {
    ...
    it('<Notification /> : notifications are rendered when notifiactionsList is not empty', () => {
        const wrapper = shallow(<Notifications />)
        wrapper.setProps({ "displayDrawer": true, "listNotifications": [{ id: 1, type: "default", value: "test" }] });
    ...
    it('first <NotificationItem /> node render the right html', () => {
        const wrapper = shallow(<Notifications />);
        wrapper.setProps({ "displayDrawer": true, "listNotifications": [{ id: 1, type: "default", value: "test" }] });
        expect(wrapper.find(".listNotificationItems").children().first().html()).toBe("<li class=\"default_dsgpx3\">test</li>")
    })
/*
